...
import { Collection, ObjectId, Document, Long } from 'mongodb'
import type {
  ${model.name},
...
export class ${model.name}Delegate {
  ...
  private readonly defaultValues = {
    ${model.fields.filter(f => f.defaultValue !== undefined || f.defaultGenerator !== undefined || (f.isRequired && f.isArray && !f.isId)).map(f => {
      ...
      if (f.defaultGenerator === 'cuid2') return `${f.name}: 'c' + crypto.randomUUID().replace(/-/g, '').toLowerCase().slice(0, 23)`;
      if (f.defaultGenerator === 'ulid') return `${f.name}: (() => { const ENC = '0123456789ABCDEFGHJKMNPQRSTVWXYZ'; let ts = Date.now(); let r = ''; for (let i = 0; i < 10; i++) { r = ENC[ts % 32] + r; ts = Math.floor(ts / 32); } const rand = crypto.randomUUID().replace(/-/g, '').toUpperCase(); for (let i = 0; i < 16; i++) { r += ENC[parseInt(rand[i % rand.length], 16) % 32]; } return r; })()`;
...
${hasBigInt ? `      const upsertData = { ...args.update, updatedAt: now }
      this.convertBigIntFields(upsertData)
      const upsertCreate = { ...args.create }
      this.convertBigIntFields(upsertCreate)` : ''}
      const result = await this.collection.findOneAndUpdate(
        query,
        {
          $set: ${hasBigInt ? 'upsertData' : '{ ...args.update, updatedAt: now }'},
          $setOnInsert: { ${hasBigInt ? '...upsertCreate' : '...args.create'}, _id: new ObjectId(), createdAt: now, updatedAt: now }
        },
